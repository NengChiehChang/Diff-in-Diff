...
for (i in 1:B){
  ...
  #CV$lambda.1se
  fit=glmnet(X[i,,],D[i,],family="binomial",alpha=1,lambda=0.079)
  beta1hat=fit$beta
...
for (i in 1:B){
  ...
  for (q in 1:k){
    ...
    #    CV$lambda.1se
    fit=glmnet(X[i,-K[q,],],D[i,-K[q,]],family="binomial",alpha=1,lambda=0.079)
    beta1hat=fit$beta
...
par(mfrow=c(1,2))
hist(thetahat,breaks=100,main="Abadie",xlab="",ylab="")
hist(thetabar[which(thetabar>2.8)],breaks=100,main="DMLDiD",xlab="",ylab="")
